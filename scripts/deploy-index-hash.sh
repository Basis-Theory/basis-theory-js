...
if [ "${ENVIRONMENT}" = pr ]; then
  JS_BUCKET_NAME="dev-bt-elements"
else
  JS_BUCKET_NAME="${ENVIRONMENT}-bt-elements"
fi
...
# Upload Content
aws s3 cp  --content-type "text/javascript" "$BUNDLE_PATH" s3://"${JS_BUCKET_NAME}"/"${INDEX_JS_NAME}"
aws s3 cp  --content-type text/plain "$JS_HASH_PATH" s3://"${JS_BUCKET_NAME}"/"${INDEX_JS_NAME}-hash"
...
if [ "$ENVIRONMENT" = pr ] ; then
  ...
  aws s3 cp --content-type "text/javascript" "$BUNDLE_PATH" s3://"${JS_BUCKET_NAME}"/"${BLOB_NAME}"
  aws s3 cp --content-type text/plain "$JS_HASH_PATH" s3://"${JS_BUCKET_NAME}"/"${BLOB_NAME}-hash"
else
  echo "Uploading bundle to $JS_HOST/$VERSIONED_JS_NAME"

  aws s3 cp --content-type "text/javascript" "$BUNDLE_PATH" s3://"${JS_BUCKET_NAME}"/"${VERSIONED_JS_NAME}"
  aws s3 cp --content-type text/plain "$JS_HASH_PATH" s3://"${JS_BUCKET_NAME}"/"${VERSIONED_JS_NAME}-hash"
fi
